...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--file", type=str, default="../senegal_test.json")
    ...
    if args.test:
        ...
        model = CamembertForSequenceClassification.from_pretrained('camembert-base', num_labels=3)
        pred = test_file('camembert_weights_{}.pth'.format(args.option), test_loader, device, model,
                         flat=False)
        test_data['pred'] = pred
        ...
        model = CamembertForSequenceClassification.from_pretrained('camembert-base', num_labels=3)
        data['pred'] = analyse('camembert_weights_{}.pth'.format(args.option), test_loader, device, model)
